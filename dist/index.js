...
const shlex = __importStar(__nccwpck_require__(659));
const configName = 'dendron.yml';
const doIgnorePrivate = (config) => {
    config.workspace.vaults = config.workspace.vaults.filter(({ visibility }) => visibility !== 'private');
    if (config.site.duplicateNoteBehavior &&
        ...
        config.site.duplicateNoteBehavior.payload =
            config.site.duplicateNoteBehavior.payload.filter((name) => nameSet.has(name));
    }
...
(async () => {
    ...
    const dendronConfig = yaml.load(dendronConfigData);
    if (ignorePrivate) {
